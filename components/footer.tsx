...
const Footer = ({side, setSide, name}:IProps) => {
    ...
    <div className={styles.footer}>
        {side && <div className={styles.footer_nav}>
            ...
            <ul>
                <Link href={`/v1/${name}`} >
                    <li className={!router.pathname.includes("categories") && !router.pathname.includes('bookmarked') && !router.pathname.includes("category") ? styles.active : ""}>
                        All Links
                    </li>
                </Link>
        ...
        <div className={styles.mobile_footer}>
            ...
            <div className={options ? styles.more_active : ''}  onClick={() => setOptions(!options)}>
                <MoreSvg/>
            </div>
        </div>
    </div>
